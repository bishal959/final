...
<form method="POST" action="atenpost.php" class="container mx-auto">
  ...
  <label>Subject:
  <input type="text" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-100" name="subject" value="<?php echo $_SESSION['selectedSubject']; ?>" readonly>
  </label>
  <label>Date:
  <input type="date" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-100" id="date" name="date" required readonly>
  </label>
